...
tCmdLineEntry g_psCmdTable[] =
{
    ...
    { "select",     Cmd_log_dir,    "Select directory that logger saves data to" },
    { 0, 0, 0 }
};
...
int
Cmd_log_dir(int argc, char *argv[] )
{
    // Get dir pointer for where to create logged files
    // The plan:
    //      parse the input string - handle the case where a full file path is not stated
    //      Check to see that the file is valid for writing to
